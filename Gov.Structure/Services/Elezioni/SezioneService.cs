...
namespace Gov.Structure.Services.Elezioni
{
    public class SezioneService : EntityService<Sezioni>, ISezioneService
    {
        ...
        public Sezioni findByNumerosezioneAndTipoelezioneId(int numeroSezione, int tipoElezioneId)
        {
                ...
                return _dbset.Include(i => i.Iscritti).Include(i => i.IdtipoelezioneNavigation).Include(i => i.IdtiposezioneNavigation).Include(i=>i.IdplessoNavigation).Include(i=>i.UsersSezioni).ThenInclude(i=>i.User).Where(x => x.Numerosezione == numeroSezione && x.IdtipoelezioneNavigation.Id == tipoElezioneId).FirstOrDefault();
